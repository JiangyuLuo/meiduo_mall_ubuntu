...
class Captcha(object):
    ...
    def initialize(self, width=200, height=75, color=None, text=None, fonts=None):
        # 字体换成可免费商用的 [os.path.join(self._dir, 'fonts', font) for font in ['GROTESKIA_OBLIQUE.otf', 'Rabiola.otf', 'Facile_Sans.ttf']]

        # self.image = Image.new('RGB', (width, height), (255, 255, 255))
        # self._text = text if text else random.sample(string.ascii_uppercase + string.ascii_uppercase + '3456789', 4)
        # print(string.ascii_letters)
        # 将所有有可能难以辨识的字母和数字去除, 防止用户读取困难
        self._text = text if text else random.sample('acdefhjkmnptuvwxyzACEFGHJKLMNPQRUVWXY' + '3479', 4)
        self.fonts = fonts if fonts else \
            [os.path.join(self._dir, 'fonts', font) for font in
             ['GROTESKIA_OBLIQUE.otf', 'Rabiola.otf', 'Facile_Sans.ttf']]
        self.width = width
        self.height = height
